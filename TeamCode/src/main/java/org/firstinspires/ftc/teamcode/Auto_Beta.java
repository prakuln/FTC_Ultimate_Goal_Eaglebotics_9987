...
public class Auto_Beta extends LinearOpMode {
    ...
    public void DriveForwardPID(double setPoint){
        ...
        while(opModeIsActive() && (((setPoint - current_state_Left) !=0) || ((setPoint - current_state_Right) !=0))){
            current_state_Left = MLeft.getCurrentPosition();
            current_state_Right = MRight.getCurrentPosition();
            double plantOutput_Left = PID.output(setPoint,current_state_Left);
            double plantOutput_Right = PID.output(setPoint,current_state_Right);
            MLeft.setPower(plantOutput_Left);
            MRight.setPower(plantOutput_Left);

            wait(delay_ms);

            telemetry.addData("state Left: ", current_state_Left);
            telemetry.addData("state Right: ", current_state_Right);
            telemetry.addData("setpoint: ", setPoint);
            telemetry.addData("error Left: ", setPoint - current_state_Left);
            telemetry.addData("error Right: ", setPoint - current_state_Right);
            telemetry.addData("plant output Left: ", plantOutput_Left);
            telemetry.addData("plant output Right: ", plantOutput_Right);
